...
export default function WithdrawReport() {
  ...
  const load = () =>
    withdrawalsApi.list({ startDate, endDate, page: currentPage, limit: pageSize })
      .then(({ requests, total: t }) => { setWithdrawals(requests); setTotal(t) })
      .catch(console.error)
  ...
  const filteredData = useMemo(() =>
    withdrawals.filter((row) =>
      !search || [row.user?.mobile, row.user?.name, row.status, String(row.amount)]
  ...
  const summaryByStatus = useMemo(() => {
    ...
    filteredData.forEach((row) => {
      ...
      if (!map[status]) map[status] = { amount: 0, count: 0 }
      map[status].amount += row.amount
      map[status].count += 1
    })
    return map
  }, [filteredData])
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold text-gray-900 mb-6">Withdraw Transactions</h2>
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <div className="p-4 border-b border-gray-200">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <h3 className="text-lg font-semibold text-gray-800">Withdraw Summary</h3>
        ...
        <div className="p-4 border-b border-gray-200">
          <div className="mb-4">
            <div className="bg-blue-600 text-white rounded-lg p-4">
              <h5 className="text-sm font-medium">Total Withdraw</h5>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {allStatuses.map((status) => {
              const { amount = 0, count = 0 } = summaryByStatus[status] || {}
              return (
          ...
          <table className="w-full text-sm text-left border-collapse border border-gray-300">
            <thead className="bg-gray-100 text-gray-700 font-semibold">
              <tr>
                ...
                <th className="px-4 py-2 border border-gray-300">Withdraw Status</th>
              ...
              {displayedData.length === 0 ? (
                ...
                <tr key={row.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2 border border-gray-300">
                    <span className={`px-3 py-1 text-xs text-white rounded ${statusColors[row.status] || 'bg-gray-600'}`}>{row.status}</span>
                  </td>
